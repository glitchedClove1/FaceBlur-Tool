...
def run_all_detections(model, device, cfg, images_root: Path, event_list, file_list, conf_thresh: float, max_detections: int):
    ...
    for ei in tqdm(range(num_events), desc="Running detection"):
        ...
        for ji in range(len(img_names)):
            ...
            boxes, scores = detect(
                image, model, device, conf_thresh=conf_thresh, nms_thresh=nms_thresh,
                image_size=size, mean=norm["mean"], std=norm["std"], variances=variances,
...
def image_eval(pred_boxes: np.ndarray, pred_scores_norm: np.ndarray, gt_boxes: np.ndarray, ignore: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
def evaluate(cfg: dict, checkpoint_path: str, conf_thresh: float = 0.01, max_detections: int = 750) -> dict[str, float]:
    ...
    for setting in SETTINGS:
        gt_list = keep_lists[setting]
        pr_curve = np.zeros((THRESH_NUM, 2))
        count_face = 0

        num_events = len(event_list)
        for ei in tqdm(range(num_events), desc=f"Scoring [{setting}]"):
            img_names = file_list[ei][0]
            sub_gt = gt_list[ei][0]
            ...
            for ji in range(len(img_names)):
                ...
                keep_index = sub_gt[ji][0].reshape(-1)
                ...
                pred_recall, proposal_list = image_eval(pred_boxes, pred_scores_norm, gt_boxes_xyxy, ignore)
                pr_curve += accumulate_pr(pred_scores_norm, proposal_list, pred_recall)
